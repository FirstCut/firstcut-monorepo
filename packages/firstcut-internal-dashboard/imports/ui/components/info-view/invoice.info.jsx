...
import {paymentMethods as collaboratorPaymentMethods} from './collaborator.info.jsx';
import {HumanReadableDate} from '../utils/dates.jsx';
import {USDollars} from '../utils/utils.jsx';
...
function _basicInfo(props) {
  ...
  const body = (
    <List>
      <Header> {record.type} FOR <a href={getRecordPath(record.gig)}>{record.gigDisplayName}</a>
      </Header>
      {record.date_paid && <List.Item>
        <List.Content>
          <b>Date Paid </b>
      ...
      <List.Item>
        <List.Content>
          <b>Amount </b>
          <USDollars amount={record.amount}/></List.Content>
      </List.Item>
      <List.Item>
        <List.Content>
          <b>Status </b> {record.status}</List.Content>
      </List.Item>
      <List.Item>
        <List.Content>
          <b>Payee </b>
          {record.payeeDisplayName}</List.Content>
      </List.Item>
      <List.Item>
        <List.Content>
          <b>Transaction Id </b>
          {record.transactionId}</List.Content>
      </List.Item>
      <List.Item>
        <List.Content>
          <b>Note </b>
          {record.note}</List.Content>
      </List.Item>
    </List>
  )
      ...
      //     {(record.createdByRecord)
      //       ? record.createdByRecord.displayName
      //       : 'Failed to find user record ' + record.createdBy}
      //   </List.Content>
      // </List.Item>
