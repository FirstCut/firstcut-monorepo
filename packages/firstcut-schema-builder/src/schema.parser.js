...
const FIELD_NAME_DELINEATOR = '.';
const ARRAY_INDEX_REGEX = /[0-9]+/;

const SchemaParser = Object.freeze({
  getFieldSchema: function(schema, fieldname) {
    return _getFieldSchema(fieldname);
  },

  getFieldLabel: function(schema, fieldname) {
    return schema.label(fieldname);
  },

  getFileStoreName: function(schema, fieldname) {
    return _getFieldSchema(schema, fieldname).store;
  },

  getFieldCustomType: function(schema, fieldname) {
    return _getFieldSchema(schema, fieldname).customType;
  },

  hasNestedFields: function(key) {
    return _hasNestedFields(key);
  },

  parseNestedFields: function(key, fields=[]) {
    return _parseNestedFields(key, fields);
  },

  getMostNestedFieldName: function(field) {
  ...
  getLeastNestedFieldName: function(field) {
    const nested_fields = _parseNestedFields(field);
    return nested_fields[0];
  },

  isObjectArrayField: function(key) {
    return _arrayIndicatorIndex(key) != -1;
  },

  buildObjectField: function(field, subfield) {
    return _fieldAsObjectKey(field) + subfield;
  },

  buildObjectArrayField: function(field, subfield) {
    return _buildObjectArrayField(field, subfield);
  },

  fieldAsObjectArrayKey: function(field) {
    return _fieldAsObjectArrayKey(field);
  },

  fieldAsIndexedObjectArrayKey: function(field, index) {
    return _fieldAsIndexedObjectArrayKey(field, index);
  },

  applyIndexToObjectArrayField: function(field, index) {
    return _applyIndexToObjectArrayField(field, index);
  },

  fieldAsObjectKey: function(field) {
    return _fieldAsObjectKey(field);
  },

  unindexObjectArrayField: function(field) {
    return _unindexObjectArrayField(field);
  },

  buildIndexedObjectArrayField: function(field, index, subfield) {
    return _buildIndexedObjectArrayField(field, index, subfield);
  },

  getIndexInObjectArrayField: function(field, index, subfield) {
