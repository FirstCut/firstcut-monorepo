...
describe('base model', () => {
  ...
  describe('mongo interface', () => {
    ...
    it('should filter by calculated fields', () => {
      let latestKeyEvent = 'SHOOT_COMPLETED';
      let query = { latestKeyEvent };
      console.log(query);
      let projects = Project.find(query);
    ...
    it('should filter by calculated fields where value is function', () => {
      let latestKeyEvent = 'SHOOT_COMPLETED';
      let query = {
        latestKeyEvent: e => e === latestKeyEvent,
      };
      let projects = Project.find(query);
      expect(projects.size).to.equal(1);
      expect(projects.get(0).latestKeyEvent).to.equal(latestKeyEvent);
      ...
      query = { latestKeyEvent: e => e !== latestKeyEvent };
      projects = Project.find(query);
      expect(projects.size).to.equal(2);
      expect(Project.count(query)).to.equal(projects.size);
      expect(Project.findOne(query).blueprint).to.be.a('string');
    ...
    it('should filter by calculated fields when key is function on model', () => {
      let query = { isWrapped: true };
      let projects = Project.find(query);
      expect(projects.size).to.equal(1);
      expect(Project.count(query)).to.equal(projects.size);

      query = { isWrapped: false };
      projects = Project.find(query);
      expect(projects.size).to.equal(2);
      expect(Project.count(query)).to.equal(projects.size);
      expect(Project.findOne(query).blueprint).to.be.a('string');
